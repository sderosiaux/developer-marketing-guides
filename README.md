# Developer Marketing Guides

A comprehensive collection of actionable guides for marketing developer tools. Distilled from 139 issues of the [Newslepear newsletter](https://newslepear.beehiiv.com/) by [Tom Redman](https://twitter.com/redman).

## What's Inside

12 in-depth guides covering every aspect of developer marketing:

| Guide | Description |
|-------|-------------|
| [01. Website & Landing Pages](guides/01-website-landing-pages.md) | Hero sections, CTAs, social proof, pricing page design |
| [02. Hacker News & Product Hunt](guides/02-hacker-news-product-hunt.md) | Launch strategies, timing, post formats, community engagement |
| [03. Paid Advertising](guides/03-paid-advertising.md) | Reddit ads, LinkedIn, YouTube influencers, Ethical Ads |
| [04. Pricing & Positioning](guides/04-pricing-positioning.md) | Pricing models, anchoring, competitive positioning |
| [05. Documentation](guides/05-documentation.md) | Docs that convert, quickstarts, API references |
| [06. Content Marketing](guides/06-content-marketing.md) | Blog strategy, SEO, technical writing, distribution |
| [07. Events & Conferences](guides/07-events-conferences.md) | Booth strategy, swag, talks, guerrilla tactics |
| [08. Community Building](guides/08-community-building.md) | Discord/Slack communities, engagement, scaling |
| [09. Sales & GTM](guides/09-sales-gtm.md) | PLG, product-led sales, enterprise motion |
| [10. Open Source & GitHub](guides/10-open-source-github.md) | Stars growth, README strategy, OSS monetization |
| [11. Social Media](guides/11-social-media.md) | Twitter/X, LinkedIn, founder-led social |
| [12. Metrics & Attribution](guides/12-metrics-attribution.md) | KPIs, self-reported attribution, ROI measurement |

## Who This Is For

- **Developer Marketing Managers** looking for proven tactics
- **DevRel professionals** building go-to-market strategies
- **Founders** of developer tools figuring out their first marketing motions
- **Product Marketers** transitioning to developer audiences

## How to Use

Each guide includes:
- **Resources & Further Reading** — Links to original sources, podcasts, and examples
- **Actionable frameworks** — Step-by-step processes you can implement
- **Real examples** — Case studies from Stripe, Supabase, PostHog, Datadog, and more
- **Templates** — Ready-to-use formats for common tasks
- **Checklists** — Quick references for execution

## Source

This knowledge base was built by consolidating insights from the [Newslepear newsletter](https://newslepear.beehiiv.com/), which curates the best resources on developer marketing since March 2022.

Key sources referenced throughout:
- [Markepear.com](https://markepear.com) — Developer marketing examples and analysis
- [DevRel.co](https://devrel.co) — Developer relations resources
- [OpenView Partners](https://openviewpartners.com) — PLG benchmarks and research
- [Heavybit](https://heavybit.com) — Developer company building

## Contributing

Found a broken link? Have a resource to add? PRs welcome.

## License

MIT
